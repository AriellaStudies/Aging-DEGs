...
```
Necessary revisions (flipped scores, white zeroes, and eliminating empty rows)
```{python}
...
plt.show()
```
Next round of revisions (y axis adjustment)
```{python}
# Assuming heatmap_df is already created and filled with the correct data
...
# Set the style and context for more readable plots
sns.set(style="white", context="talk")

# Create the heatmap
plt.figure(figsize=(6, 10),dpi=300)  # This should now reflect correctly in the output
...
# Ensure the whole heatmap is visible and labels are not cut off
plt.xticks(rotation=90)  # Rotate x-axis labels for better readability
plt.yticks(rotation=0)
plt.subplots_adjust(left=0.3, bottom=0.1)  # Adjust spacing to ensure everything fits within the plot area
plt.title('Comprehensive Gene Expression Over/Under Regulation by Tissue')
plt.xlabel('Tissues')
plt.ylabel('Genes')
plt.tight_layout()
plt.show()
plt.savefig('comprehensive_heatmap.svg')
...
# Reset to default settings to clear any previous customizations
sns.reset_orig()
...
# Ensure the whole heatmap is visible and labels are not cut off
plt.subplots_adjust(left=0.3, bottom=0.1, top=0.9, right=0.95)  # Fine-tune these parameters

# Reduce font size by about 30-40%
title_fontsize = 16 * 0.7  # reduce title font size by 30%
label_fontsize = 14 * 0.7  # reduce axis labels font size by 30%
tick_fontsize = 12 * 0.7  # reduce tick labels font size by 30%

# Set title and labels with explicit size to ensure they fit and are readable
ax.set_title('Comprehensive Gene Expression Over/Under Regulation by Tissue', fontsize=16)
ax.set_xlabel('Tissues', fontsize=14)
ax.set_ylabel('Genes', fontsize=14)

# Manually adjust tick parameters if automatic adjustment fails
ax.tick_params(axis='both', which='major', labelsize=12)

# Use tight layout to ensure that all plot components fit within the figure area
plt.tight_layout()
...
# Set the style for readability
sns.set(style="white", context="talk")

# Define figure and DPI with adjusted height
plt.figure(figsize=(6, 20), dpi=300)  # Increased vertical size to accommodate labels
...
# Set custom y-ticks to ensure all labels are shown
ax.set_yticks(range(len(heatmap_df.index)))
ax.set_yticklabels(heatmap_df.index, rotation=0)  # Ensure labels are horizontal

# Adjust font sizes for better visibility and fit
plt.title('Comprehensive Gene Expression Over/Under Regulation by Tissue', fontsize=14)
plt.xlabel('Tissues', fontsize=12)
plt.ylabel('Genes', fontsize=12)
ax.tick_params(axis='both', which='major', labelsize=10)

# Adjust layout padding to avoid cutting off elements and to handle squishing
plt.subplots_adjust(left=0.3, bottom=0.1, top=0.95, right=0.95)
