...
Other Packages
```{r eval=FALSE}
packages <- c('limma', 'umap', 'reticulate', 'homologene', 'ggplot2', 'dplyr', 'svglite')
...
```{python}
path6down = total_df[total_df.iloc[:,0]>=6]
path6up = total_df[total_df.iloc[:,0]<=-6]
len(path6down)
len(path6up)
```
Print a few quick counts for results section
```{python}
old7 = old_counts[old_counts.iloc[:, 0] >= 7]
total7plus = ordered_df[ordered_df.iloc[:,0]<=-7]
young7 = young_counts[young_counts.iloc[:,0]>=7]
total7minus = ordered_df[ordered_df.iloc[:,0]>=7]

print(f"The # of genes with an upregulation score of at least 7 was {len(old7)}.")
print(f"The # of upregulated genes with a total rank of at least 7 was {len(total7plus)}.")

print(f"The # of genes with a downregulation score of at least 7 was {len(young7)}.")
print(f"The # of downregulated genes with a total rank of at least 7 was {len(total7minus)}.")

```

```{python}
old6 = old_counts[old_counts.iloc[:, 0] >= 6]
total6plus = ordered_df[ordered_df.iloc[:,0]<=-6]
young6 = young_counts[young_counts.iloc[:,0]>=6]
total6minus = ordered_df[ordered_df.iloc[:,0]>=6]

print(f"The # of genes with an upregulation score of at least 6 was {len(old6)}.")
print(f"The # of upregulated genes with a total rank of at least 6 was {len(total6plus)}.")

print(f"The # of genes with a downregulation score of at least 6 was {len(young6)}.")
print(f"The # of downregulated genes with a total rank of at least 6 was {len(total6minus)}.")
```
```{python}
total6 = len(total6plus) + len(total6minus)

total7 = len(total7plus) + len(total7minus)

print(f"The total for rank 6 is {total6}, and the total for rank 7 is {total7}")
```
 
 Trying a binomial statistical test with chatGPT's help (but I don't really think this was that helpful)
```{python}
# Import necessary libraries
from scipy.stats import binom

# Initialize the parameters
n_datasets = 25  # total number of datasets you analyzed
p = 0.05  # traditional baseline probability for a single hypothesis test

# Initialize the dictionary to store significant genes
significant_genes = {}

# Conduct the binomial test on each gene in the total_dict
for gene, count in total_dict.items():
    # Calculate the p-value using the binomial distribution
    p_value = 1 - binom.cdf(count - 1, n_datasets, p)
    
    # Check if the p-value is less than the significance level
    if p_value < 0.05:
        # If significant, store the gene with its count and p-value
        significant_genes[gene] = (count, p_value)

# Output the significant genes and their details
for gene, details in significant_genes.items():
    print(f"Gene: {gene}, Count: {details[0]}, P-value: {details[1]:.4f}")

```
 

## 4B. Trial: Enhanced version of section 4A function for use in later analysis

Enhanced Reader Function
```{python}
...
Executing Enhanced Reader Function
```{python}
...
## 4C. Querying Individual DEG Scores
...
## 4D. Show dataset characteristics for each high-scoring gene
Show genes with high scores
*Note: adjust HERE to change the score limit for section 4D*
```{python}
...
#Human
#tissues_for_hits('tT007.txt') excluded as <3 DEGs
...
plt.show()
```
## 4E. WIP: GPT Assisted Split Up/Down Analysis and Viz Based on 4B
TRY STARTING HERE AFTER LOADING PYTHON LIBRARIES
| NEW PLOT STEP NUMBA1 - Prepare function to make foundational dictionary|
```{python}
...
Apply the function
| NEW PLOT STEP NUMBA2 - USE tT FILES|
```{python}
...
SKIPPABLE: Just checking things
```{python}
...
```
SKIPPABLE: Print this for just for reference when using the next chunk:
```{python}
# Define the total datasets per tissue based on your data
total_datasets_per_tissue = {
    'Brain': 5,
    'Cochlea': 1,
    'Fat': 3,
    'Heart': 2,
...
for tissue in total_datasets_per_tissue:
    tcount = total_datasets_per_tissue[tissue]
    print(f"{tissue}:{tcount}")
```

SKIPPABLE: Apply it to genes of interest specifically
```{python}
# List of your specific genes of interest
genes_of_interest = ['SPARC', 'DIRC2', 'CA4', 'CDC20', 'RSRC1', 'CASP1']
#genes_of_interest = ['PTPRC','APOE','CASP1', 'RTN4IP1','SKAP2']
#genes_of_interest = ['HLA-A','BRWD1','EIF4EBP1','CTSS','RTN4IP1','NPC2','SERPINB6','SFRP1','TMEM176A','ANXA4','ARAP2','UQCR10','NREP']
#genes_of_interest = ['EFEMP1','TMEM176A']
...
plt.show()
```
Version with color-scale only, no numbers inside the graph
```{python}
# Assuming heatmap_df is the DataFrame that combines the upregulated and downregulated counts
# Fill NaN values with 0 to ensure that empty entries are treated as zeros
heatmap_df = heatmap_df.fillna(0)
...
Try using a similar process to execute a similar heatmap for all high-ranking genes
| NEW PLOT STEP NUMBA3 - use GENE LIST  | 
```{python}
# List of all 45 genes from your meta-analysis
genes_of_interest = [
    "UQCR10", "SIAH2", "CA4", "SPARC", "BRWD1", "BRD3", "SMYD1", "EIF4EBP1", "CDC20", "AR", "VLDLR", "DIRC2",
    "KIFAP3", "EIF2D", "NREP", "RTN4IP1", "CASP1", "CTSS", "RSRC1", "FST", "SLC44A1", "CTNNA1", "C1QC", "PJA2",
    "LYST", "FYB", "SKAP2", "RNF145", "EHD4", "CFLAR", "TMEM123", "SERPINB6", "MGST1", "NPC2", "SFRP1",
    "APOE", "ARAP2", "PTPRC", "ANXA4", "GCNT2", "PRNP", "HLA-A", "CP", "TMEM176A", "EFEMP1"
]
```


Use this version if you want to make a plot without any normalization, otherwise proceed to step 4
```{python}
# Initialize a dictionary to hold the data for upregulated and downregulated datasets
heatmap_data = {gene: {'upregulated': {}, 'downregulated': {}} for gene in genes_of_interest}
...
# Convert the dictionary to DataFrames for upregulated and downregulated data
df_upregulated = pd.DataFrame({gene: heatmap_data[gene]['upregulated'] for gene in genes_of_interest}).fillna(0)
df_downregulated = pd.DataFrame({gene: heatmap_data[gene]['downregulated'] for gene in genes_of_interest}).fillna(0)
...
heatmap_df = df_upregulated + df_downregulated
heatmap_df = heatmap_df.T  # Transpose the DataFrame to switch genes to y-axis and tissues to x-axis
# Flip the sign of the upregulated and downregulated data to align with new conventions
heatmap_df = -1 * heatmap_df
```
...
# Explicitly define the figure and axes for better control
...
# Create the heatmap
sns.heatmap(heatmap_df, ax=ax, cmap=cmap, annot=False, center=0, linewidths=.5, linecolor='grey')

# Set y-ticks more explicitly and adjust alignment
ax.set_yticks(range(len(heatmap_df.index)))
ax.set_yticklabels(heatmap_df.index, fontsize=12, va='center', ha='right')  # Adjust font size and alignment

ax.set_title('DEGs by Tissue Type')
...
# Adjust subplot parameters to ensure no clipping
fig.subplots_adjust(left=0.3, right=0.95, top=0.95, bottom=0.1)

# Use tight layout with padding to make sure everything fits without overlap
fig.tight_layout(pad=3.0)

plt.savefig('heatmapZ2.png', dpi=300)
plt.show()


```
| NEW PLOT STEP NUMBA4A - add TISSUE list to normalize ORIGINAL METHOD, SKIP TO 4B IF WANT "Other" METHOD | 
Let's try normalizing to total number datasets
```{python}
import pandas as pd
import seaborn as sns
import matplotlib.pyplot as plt
...
# Normalize the data
normalized_data = {gene: {'upregulated': {}, 'downregulated': {}} for gene in genes_of_interest}
for gene in genes_of_interest:
    if gene in accumulated_gene_details:
        for tissue in heatmap_data[gene]['upregulated']:
            if tissue in total_datasets_per_tissue:
                normalized_value = heatmap_data[gene]['upregulated'][tissue] / total_datasets_per_tissue[tissue]
                normalized_data[gene]['upregulated'][tissue] = normalized_value
        for tissue in heatmap_data[gene]['downregulated']:
            if tissue in total_datasets_per_tissue:
                normalized_value = heatmap_data[gene]['downregulated'][tissue] / total_datasets_per_tissue[tissue]
                normalized_data[gene]['downregulated'][tissue] = normalized_value

# Convert the normalized dictionary to DataFrame for visualization
df_normalized_upregulated = pd.DataFrame({gene: normalized_data[gene]['upregulated'] for gene in genes_of_interest}).fillna(0)
df_normalized_downregulated = pd.DataFrame({gene: normalized_data[gene]['downregulated'] for gene in genes_of_interest}).fillna(0)

# Combine normalized upregulated and downregulated counts
df_normalized_upregulated *= 1
df_normalized_downregulated *= -1
normalized_heatmap_df = df_normalized_upregulated + df_normalized_downregulated
normalized_heatmap_df = normalized_heatmap_df.T  # Transpose if necessary
# Flip the sign of the upregulated and downregulated data to align with new conventions
normalized_heatmap_df = -1 * normalized_heatmap_df
```

| NEW PLOT STEP NUMBA4B - ALT: ADD TISSUE list with "OTHER" Mod
```{python}
#STEP 4: NORMALIZE WITH TISSUE DATA
import pandas as pd
import seaborn as sns
import matplotlib.pyplot as plt

# Initialize a dictionary to hold the data for upregulated and downregulated datasets
tissues_list = ['Brain', 'Fat', 'Heart', 'Immune', 'Liver', 'Muscle', 'Other']  # Include 'Other' and all other tissues
heatmap_data = {
    gene: {'upregulated': {tissue: 0 for tissue in tissues_list},
           'downregulated': {tissue: 0 for tissue in tissues_list}}
    for gene in genes_of_interest
}
...
total_datasets_per_tissue = {
    ...
    'Liver': 2,
    'Muscle': 7,
    'Other': 3  # Combining Cochlea, Reproduction, and Trachea
}

# Populate the dictionary from accumulated_gene_details with merging specific tissues into 'Other'
for gene in genes_of_interest:
    if gene in accumulated_gene_details:
        for dataset in accumulated_gene_details[gene]['upregulated_datasets']:
            tissue = dataset['tissue']
            if tissue in ['Cochlea', 'Reproduction', 'Trachea']:  # Convert to 'Other'
                tissue = 'Other'
            heatmap_data[gene]['upregulated'][tissue] += 1  # Properly accumulate data first

        for dataset in accumulated_gene_details[gene]['downregulated_datasets']:
            tissue = dataset['tissue']
            if tissue in ['Cochlea', 'Reproduction', 'Trachea']:
                tissue = 'Other'
            heatmap_data[gene]['downregulated'][tissue] += 1


# Normalize the data after accumulation
normalized_data = {gene: {'upregulated': {}, 'downregulated': {}} for gene in genes_of_interest}
for gene in genes_of_interest:
    for tissue in tissues_list:
        if tissue in total_datasets_per_tissue:  # Only normalize if the tissue has datasets
            if tissue in heatmap_data[gene]['upregulated']:
                normalized_data[gene]['upregulated'][tissue] = heatmap_data[gene]['upregulated'][tissue] / total_datasets_per_tissue[tissue]
            if tissue in heatmap_data[gene]['downregulated']:
                normalized_data[gene]['downregulated'][tissue] = heatmap_data[gene]['downregulated'][tissue] / total_datasets_per_tissue[tissue]
...
Let's try making the heatmap again the same way as previously but with this new data
```{python}
import matplotlib.pyplot as plt
import seaborn as sns
...
# Create the heatmap
sns.heatmap(normalized_heatmap_df, ax=ax, cmap=cmap, annot=False, center=0, linewidths=.5, linecolor='grey')

# Set y-ticks more explicitly and adjust alignment
ax.set_yticks(range(len(normalized_heatmap_df.index)))
ax.set_yticklabels(normalized_heatmap_df.index, fontsize=12, va='center', ha='right')  # Adjust font size and alignment
...
plt.savefig('normalheatmapZ2.png', dpi=300)
plt.show()
```
Try a different format for SVG export
| NEW PLOT STEP NUMBA5 - PLot Heatmap | 
```{python}
import matplotlib.pyplot as plt
import seaborn as sns
...
# Set y-ticks more explicitly and adjust alignment
ax.set_yticks(range(len(normalized_heatmap_df.index)))
ax.set_yticklabels(normalized_heatmap_df.index, fontsize=12, va='center', ha='right')
...
# Adjust subplot parameters manually without using tight_layout
fig.subplots_adjust(left=0.3, right=0.95, top=0.95, bottom=0.1)

# Save the figure in different formats
plt.savefig('normalheatmapZ2.svg', format='svg', dpi=300)  # Specify format and dpi for SVG
plt.show()

```
Let's try normalizing the small one now:
```{python}
import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
...
# Save the figure in different formats
plt.savefig('normalSMOLheatmap.svg', format='svg', dpi=300)  # Specify format and dpi for SVG
plt.show()
```

Let's try visualizing by species now
```{python}
import pandas as pd
import seaborn as sns
import matplotlib.pyplot as plt

total_datasets_per_species = {
    "Homo sapiens": 6,
    "Mus musculus": 13,
    "Rattus norvegicus": 5,
...
# Initialize a dictionary to hold the data for upregulated and downregulated datasets by animal
heatmap_data = {gene: {'upregulated': {}, 'downregulated': {}} for gene in genes_of_interest}

# Populate the dictionary with data from accumulated_gene_details focusing on animals 
for gene in genes_of_interest:
    if gene in accumulated_gene_details:
        for dataset in accumulated_gene_details[gene]['upregulated_datasets']:
            animal = dataset['animal']
            if animal not in heatmap_data[gene]['upregulated']:
                heatmap_data[gene]['upregulated'][animal] = 0
            heatmap_data[gene]['upregulated'][animal] += 1

        for dataset in accumulated_gene_details[gene]['downregulated_datasets']:
            animal = dataset['animal']
            if animal not in heatmap_data[gene]['downregulated']:
                heatmap_data[gene]['downregulated'][animal] = 0
            heatmap_data[gene]['downregulated'][animal] += 1

#ADD THIS FOR NORMALIZATION OR COMMENT OUT FOR UNNORMALIZED DATA:
for gene in genes_of_interest:
    for animal in total_datasets_per_species:
        if animal in heatmap_data[gene]['upregulated']:
            heatmap_data[gene]['upregulated'][animal] /= total_datasets_per_species[animal]
        if animal in heatmap_data[gene]['downregulated']:
            heatmap_data[gene]['downregulated'][animal] /= total_datasets_per_species[animal]


# Convert the dictionary to DataFrames for upregulated and downregulated data by animal
df_upregulated = pd.DataFrame({gene: heatmap_data[gene]['upregulated'] for gene in genes_of_interest}).fillna(0)
df_downregulated = pd.DataFrame({gene: heatmap_data[gene]['downregulated'] for gene in genes_of_interest}).fillna(0)

# Combine upregulated and downregulated counts in a single DataFrame
# Assign positive values for upregulated and negative values for downregulated
df_upregulated *= -1   # Optional: Use actual counts if needed
df_downregulated *= 1
heatmap_df = df_upregulated + df_downregulated
heatmap_df = heatmap_df.T  # Transpose the DataFrame to switch genes to y-axis and animals to x-axis
...
cmap = sns.diverging_palette(220, 20, as_cmap=True, center="light")
ax = sns.heatmap(heatmap_df, cmap=cmap, annot=False, center=0, linewidths=.5, linecolor='grey')
plt.xticks(rotation=45)
plt.yticks(rotation=0)
plt.subplots_adjust(left=0.2, bottom=0.1)
plt.title('Gene Expression Over/Under Regulation by Animal Type')
plt.xlabel('Animals')
plt.ylabel('Genes')
plt.show()
```
...
#Human:
#DS<-DSS_Function(tT007,"7") try excluding since it's only 2 genes that didn't pan out anyway and this will make even 25 (GSE58015)
