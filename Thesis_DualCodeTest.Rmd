...
```{python}
subset_counts = ordered_df[(ordered_df[0]>3) | (ordered_df[0]<-3)]
subset_counts
```
Show tissue types for high-scoring genes
```{python}
gene_index = subset_counts.index
gene_index = list(gene_index)
#for gene in gene_index:
#  gene_complex_dictionary[gene]=
#for item in gene_index:
#  print(item) #PLACE FUNCTION CODE HERE?
```
WORK AREA:
```{python}
...
def tissues_for_hits(file_name):
  ...
  for gene in gene_index:
    if gene in geneIDs and gene not in gene_complex_dictionary:
      gene_complex_dictionary[gene] = [indices[file_index],tissues[file_index],animals[file_index]]
...
for key in gene_complex_dictionary:
  print(key, gene_complex_dictionary[key])
```
...
**NOTE** Before running these next sessions, it's necessary to manually fill in the # DEGs column in the datasets csv file in excel. Can make this automatic in the future, but while the datasets are in flux, it is easier to manage this part manually.
