...
```
# WRITING/EXPORTING AREA
```{r}
write.table(tT, file="R_Out_1.26.txt", row.names=F, sep="\t")
write.csv(tT, file="R_Out_C.csv", row.names=F)
```

#SUB IN VALUES TESTING AREA
```{r}

GSE_number = "GSE38718"
platform_code = "GPL570"
group_binaries = "0000000000000011111111"
...
```{r}
GEO2R <- function(GSE_number,platform_code,group_binaries){
  # load series and platform data from GEO
  gset <- getGEO(GSE_number, GSEMatrix =TRUE, AnnotGPL=TRUE)
  if (length(gset) > 1) idx <- grep(platform_code, attr(gset, "names")) else idx <- 1
  gset <- gset[[idx]]
  ...
  # compute statistics and table of top significant genes
  fit2 <- eBayes(fit2, 0.01)
  tT <- topTable(fit2, adjust="fdr", sort.by="B", number=47)
...
#Execute Function
```{r}
tT01 <- GEO2R("GSE38718","GPL570","0000000000000011111111")
```
