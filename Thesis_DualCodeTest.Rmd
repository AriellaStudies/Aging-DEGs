...
Import counts data and arrange in dataframes and lists
```{r}
...
YoungAndTotal = merge(TotalCounts_byGene,YoungCounts_byGene)
OldAndTotal = merge(TotalCounts_byGene,OldCounts_byGene)
AllCounts = merge(YoungAndTotal,OldCounts_byGene,by="Gene",all.x = TRUE,all.y = TRUE)
```

Histogram of counts
```{r}
ggplot(TotalCounts_byGene, aes(x=Total_Count)) +
...
```{r}
ggplot(AllCounts, aes(x=x) ) +
  geom_histogram( aes(x = Young_Count, y = ..density..), fill="#69b3a2", binwidth = 1 ) +
  geom_label( aes(x=4.5, y=0.25, label="variable1"), color="#69b3a2") +
  geom_histogram( aes(x = Old_Count, y = -..density..), fill= "#404080" , binwidth = 1 ) +
